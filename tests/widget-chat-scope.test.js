...
executeTest('widgetChatListScope allows matching non-SDK chats', () => {
  ...
  assert.equal(widgetChatListScope(createChat({ agentTransport: 'codebuddy' }), ACCESS), true);
  assert.equal(widgetChatListScope(createChat({ agentTransport: 'deepseek' }), ACCESS), true);
  assert.equal(widgetChatListScope(createChat({ agentTransport: 'codex' }), ACCESS), true);
  assert.equal(widgetChatListScope(createChat({ agentTransport: 'qwen' }), ACCESS), true);
});
