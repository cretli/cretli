...
const workspaces = [
  ...
  { name: 'cursor-remote', workspaceFile: '/home/user/www/cursor-remote/cretli.code-workspace' },
...
test('picks a workspace by its file stem', () => {
  ...
  assert.equal(actual.match?.name, 'cursor-remote');
});
...
test('returns no match for an unknown name', () => {
  const actual = matchWorkspaceBySpokenName(workspaces, 'obsidian');
