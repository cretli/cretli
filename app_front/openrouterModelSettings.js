/**
 * Settings UI: OpenRouter model catalog and chat visibility toggles (Harness tab).
 */
 ...
 */
function buildCatalogFromOpenRouterModels(models) {
...
function readSettingsSortMode() {
  try {
    const stored = readStorageValueWithAlias(localStorage, OPENROUTER_MODEL_SETTINGS_SORT_LS_KEY, '');
...
function persistSettingsSortMode(mode) {
  ...
  try {
    writeStorageValueWithAlias(localStorage, OPENROUTER_MODEL_SETTINGS_SORT_LS_KEY, settingsSortMode);
  } catch {
    /* ignore */
  }
}
...
function syncSortSelectUi() {
  const sortEl = document.getElementById('openrouter-model-settings-sort');
...
function renderModelSettingsList() {
  const listEl = document.getElementById('openrouter-model-settings-list');
  const searchInput = document.getElementById('openrouter-model-settings-search');
  if (!listEl) return;
  const query = searchInput instanceof HTMLInputElement ? searchInput.value : '';
  const visibleRows = filterCatalogForSearch(query);
  if (visibleRows.length === 0) {
    listEl.innerHTML = '<p class="settings-hint">' + escapeHtml(t('settings.harnessOpenRouterModelsEmpty')) + '</p>';
...
function updateToggleAllButtonLabel() {
  const btn = document.getElementById('openrouter-model-settings-toggle-all-btn');
  if (!btn) return;
...
function updateModelSettingsSummary() {
  const summaryEl = document.getElementById('openrouter-model-settings-summary');
  if (!summaryEl) return;
...
function readDraftEnabledKeysFromUi() {
  ...
  /** @type {Set<string>} */
  const next = new Set();
  listEl.querySelectorAll('.openrouter-model-settings-checkbox[data-model-value]').forEach((el) => {
...
async function loadOpenRouterModelSettingsData() {
  ...
  try {
    ...
    if (!modelsData?.ok) {
      ...
      if (statusEl) {
        statusEl.textContent = modelsData?.error || t('settings.harnessOpenRouterModelsEmpty');
      }
      return;
    }
    settingsModelCatalog = enrichCatalogEntryMetaList(
      buildCatalogFromOpenRouterModels(modelsData.models),
    );
    const enabledFromSettings = Array.isArray(settingsData?.openrouterChatEnabledModels)
      ? settingsData.openrouterChatEnabledModels
...
/**
 * Initializes OpenRouter model settings panel (Settings → Harness).
 */
export function initOpenRouterModelSettings() {
  const listEl = document.getElementById('openrouter-model-settings-list');
  const saveBtn = document.getElementById('openrouter-model-settings-save-btn');
  const toggleAllBtn = document.getElementById('openrouter-model-settings-toggle-all-btn');
  const clearBtn = document.getElementById('openrouter-model-settings-clear-btn');
  const searchInput = document.getElementById('openrouter-model-settings-search');
  ...
  listEl.addEventListener('change', (event) => {
    ...
    if (!target.classList.contains('openrouter-model-settings-checkbox')) return;
  ...
  saveBtn.addEventListener('click', () => {
    ...
    api.patchSettings({ openrouterChatEnabledModels: payload }).then((data) => {
      if (!data?.ok) {
        if (statusEl) statusEl.textContent = data?.error || t('settings.chatModelsSaveError');
        return;
      }
      const savedKeys = data.openrouterChatEnabledModels || payload;
      if (statusEl) statusEl.textContent = t('settings.chatModelsSaved');
      window.dispatchEvent(new CustomEvent('cretli-openrouter-models-changed', {
        detail: { openrouterChatEnabledModels: savedKeys },
      }));
    }).catch(() => {
...
export function refreshOpenRouterModelSettingsPanel() {
  void loadOpenRouterModelSettingsData();
}
