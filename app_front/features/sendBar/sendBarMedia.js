...
import { applyPagePickMenuVisibility } from './sendBarPagePickMenu.js';
...
export function createSendBarMedia(options) {
  ...
  function stopDictation() {
    ...
    resumeDictationAfterSend = getStoredDictationResumeAfterSend();
...
function initScreenshotControls(options) {
  ...
  screenshotMenu.innerHTML =
    ...
    '<li class="chat-list-item send-keys-screenshot-menu-item" role="menuitem" data-action="page-pick" aria-disabled="true" hidden>' +
  ...
  const refreshPagePickMenuState = () => {
    ...
    applyPagePickMenuVisibility({
      item: pagePickMenuItem,
      label: pagePickMenuLabel,
      enabled: typeof hostPickFn === 'function',
      enabledLabel: t('sendBar.pickPageElement'),
    });
  };
