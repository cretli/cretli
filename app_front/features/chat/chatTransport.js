...
function resolveSdkRunFailureNotice(msg) {
  ...
  if (code === 'cursor_auth_error') {
    return t('chat.sdkAuthError', {
      detail: failureDetail || String(msg.lastErrorMessage || '').trim(),
    });
  }
    ...
    socket.onmessage = (ev) => {
      ...
      try {
        ...
        if (msg.type === 'sdkRoomState') {
          ...
          chat._sdkServerPendingQuestionCount = Math.max(0, Number(msg.pendingQuestionCount) || 0);
          chat._sdkServerPendingPermissionCount = Math.max(0, Number(msg.pendingPermissionCount) || 0);
          if (normalizeHarnessTransport(chat?.agentTransport) === 'opencode') {
            const isAwaitingOpenCodeInput =
              chat._sdkServerPendingQuestionCount > 0 || chat._sdkServerPendingPermissionCount > 0;
            if (isAwaitingOpenCodeInput) {
        ...
        if (
          msg.type === 'hello'
          && (msg.transport === 'cursor-sdk' || msg.transport === 'openrouter' || msg.transport === 'opencode')
        ) {
          chat.agentTransport = msg.transport === 'cursor-sdk' ? 'sdk' : msg.transport;
          syncSdkEventStream(chat, msg.eventStreamId);
        ...
        if (msg.type === 'sdkError') {
          ...
          if (chat._sdkRichView) {
            if (msg.code === 'run_stuck_auto_recovery') {
              ...
              chat._sdkRichView.appendError(msg.message || msg.code || 'unknown');
            }
          } else {
            appendSdkTermChunk(
              chat,
