...
export function createSidebarView(deps) {
  ...
  function renderWorkspaceGroup(workspace, activeWorkspaceFile, activeWorkspaceFolder, activeChatId) {
    ...
    const isCollapsed = isWorkspaceCollapsed(sidebarKey);
    const chats = orderedChats(chatsForWorkspace(workspace));
  ...
  function render() {
    ...
    if (!wsList.length) {
      body.innerHTML =
        '<div class="sidebar-empty">' + escapeHtml(t('workspace.emptyHint')) + '</div>';
      return;
    }
    ...
    body.innerHTML =
      ...
      ordered
        .map((w) => renderWorkspaceGroup(w, activeWorkspaceFile, activeWorkspaceFolder, activeChatId))
        .join('') +
      '</ul>';
