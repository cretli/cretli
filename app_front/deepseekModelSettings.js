/**
 * Settings UI: DeepSeek Harness model catalog and chat visibility toggles.
 */
 ...
 */
function buildCatalogFromDeepSeekModels(models) {
...
function readSettingsSortMode() {
  try {
    const stored = readStorageValueWithAlias(localStorage, DEEPSEEK_MODEL_SETTINGS_SORT_LS_KEY, '');
...
function persistSettingsSortMode(mode) {
  ...
  try {
    writeStorageValueWithAlias(localStorage, DEEPSEEK_MODEL_SETTINGS_SORT_LS_KEY, settingsSortMode);
  } catch {
    /* ignore */
  }
}
...
function syncSortSelectUi() {
  const sortEl = document.getElementById('deepseek-model-settings-sort');
...
function renderModelSettingsList() {
  const listEl = document.getElementById('deepseek-model-settings-list');
  const searchInput = document.getElementById('deepseek-model-settings-search');
  if (!listEl) return;
  const query = searchInput ? String(searchInput.value || '') : '';
  const visibleRows = filterCatalogForSearch(query);
  if (visibleRows.length === 0) {
    listEl.innerHTML = '<p class="settings-hint">' + escapeHtml(t('settings.harnessDeepSeekModelsEmpty')) + '</p>';
...
function updateToggleAllButtonLabel() {
  const btn = document.getElementById('deepseek-model-settings-toggle-all-btn');
  if (!btn) return;
...
function updateModelSettingsSummary() {
  const summaryEl = document.getElementById('deepseek-model-settings-summary');
  if (!summaryEl) return;
...
function readDraftEnabledKeysFromUi() {
  ...
  /** @type {Set<string>} */
  const next = new Set();
  listEl.querySelectorAll('.deepseek-model-settings-checkbox[data-model-value]').forEach((el) => {
...
async function refreshDeepSeekStatusPanel() {
  const statusEl = document.getElementById('deepseek-harness-status');
  if (!statusEl) return;
  try {
    const data = await api.getDeepSeekStatus();
    if (!data?.deepseekApiKeyEffective) {
      statusEl.textContent = t('settings.harnessDeepSeekNotReadyNoKey');
      return;
    }
    if (!data?.ok || !data.ready) {
      statusEl.textContent = data?.error || t('settings.harnessDeepSeekNotReady');
      return;
    }
    statusEl.textContent = t('settings.harnessDeepSeekReady');
  } catch {
    statusEl.textContent = t('settings.harnessDeepSeekNotReady');
  }
}

async function loadDeepSeekModelSettingsData() {
  ...
  void refreshDeepSeekStatusPanel();
  try {
    const settingsData = await api.getSettings();
    const modelsData = await api.getDeepSeekModels();
    if (!modelsData?.ok) {
      ...
      if (statusEl) {
        statusEl.textContent = modelsData?.error || t('settings.harnessDeepSeekModelsEmpty');
      }
      return;
    }
    settingsModelCatalog = enrichCatalogEntryMetaList(
      buildCatalogFromDeepSeekModels(modelsData.models),
    );
...
/**
 * Initializes DeepSeek model settings panel (Settings → Harness).
 */
export function initDeepSeekModelSettings() {
  const listEl = document.getElementById('deepseek-model-settings-list');
  const saveBtn = document.getElementById('deepseek-model-settings-save-btn');
  const toggleAllBtn = document.getElementById('deepseek-model-settings-toggle-all-btn');
  const clearBtn = document.getElementById('deepseek-model-settings-clear-btn');
  const searchInput = document.getElementById('deepseek-model-settings-search');
  ...
  window.addEventListener('cretli-deepseek-key-changed', () => {
    void loadDeepSeekModelSettingsData();
  });
  listEl.addEventListener('change', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLInputElement)) return;
    if (!target.classList.contains('deepseek-model-settings-checkbox')) return;
  ...
  saveBtn.addEventListener('click', () => {
    ...
    api.patchSettings({ deepseekChatEnabledModels: payload }).then((data) => {
      ...
      const savedKeys = data.deepseekChatEnabledModels || payload;
      if (statusEl) statusEl.textContent = t('settings.chatModelsSaved');
      window.dispatchEvent(new CustomEvent('cretli-deepseek-models-changed', {
        detail: { deepseekChatEnabledModels: savedKeys },
      }));
    }).catch(() => {
...
export function refreshDeepSeekModelSettingsPanel() {
  void loadDeepSeekModelSettingsData();
}
