...
import { createPanelRouter, readRequestedChatId } from './app/appShell/panelRouter.js';
...
const MAIN_PANELS = ['chat', 'terminal', 'tasks', 'agents', 'todo', 'files', 'git', 'github', 'logs', 'instances', 'tests', 'widget'];

const APP_MODES = {
  main: {
    id: 'main',
    allowedPanels: MAIN_PANELS,
    defaultPanel: 'chat',
  },
  embed: {
    id: 'embed',
...
const panelRouter = createPanelRouter({
  ...
  refreshTodoList,
  refreshWidgetPanel: () => callLoadedPanel('widget', 'refreshWidgetPanel'),
...
function initHeaderSettings() {
  ...
  if (settingsBtn) {
    settingsBtn.addEventListener('click', () => {
      ...
      initWorkspaceHeader();
      refreshSettingsWorkspacePicker();
      const activeSettingsTab = document.querySelector('.settings-tab.active')?.dataset?.settingsTab;
      if (activeSettingsTab === 'harness') {
        refreshHarnessSettingsPanel();
        refreshModelSettingsPanel();
        refreshOpenRouterModelSettingsPanel();
        refreshOpenCodeModelSettingsPanel();
      }
      if (activeSettingsTab === 'chat') refreshModelSettingsPanel();
  ...
  api.getAuthStatus()
    .then((data) => {
      if (data && data.ok && data.configured) {
        ...
        const activeTab = document.querySelector('.settings-tab.active')?.dataset.settingsTab;
...
const SETTINGS_TABS = ['workspace', 'connection', 'harness', 'interface', 'browser', 'chat', 'widgets', 'usage', 'account'];
...
function applySettingsTab(tabId) {
  ...
  document.querySelectorAll('.settings-section[data-settings-tab]').forEach((section) => {
    ...
    section.hidden = section.dataset.settingsTab !== tabId;
  });
  try {
    writeStorageValueWithAlias(localStorage, SETTINGS_TAB_LS_KEY, tabId);
  } catch (_) {}
...
function initSettingsOpenWidgetPanelButton() {
  const button = document.getElementById('settings-open-widget-panel-btn');
  if (!button) return;
...
function initSettingsTabs() {
  ...
  try {
    const stored = readStorageValueWithAlias(localStorage, SETTINGS_TAB_LS_KEY, '');
...
function initPanelCopyButtons() {
  async function handleCopy(getTerm) {
    ...
    if (ok && this instanceof HTMLButtonElement) {
...
function initMultilineToggleButton() {
  const wrap = document.getElementById('chat-extra-bar-wrap');
  if (!wrap) return;
  const row = document.createElement('div');
  row.className = 'send-bar-options-row';
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'multiline-toggle-btn send-bar-extra-action-btn';
    function updateLabel() {
      const bar = getActiveSendBarForPanel();
      const on = bar ? bar.isMultiline() : false;
      btn.textContent = on ? t('app.multilineOn') : t('app.multilineOff');
      btn.title = on
        ? t('app.multilineOnTitle')
        : t('app.multilineOffTitle');
    }
  updateLabel();
  btn.addEventListener('click', () => {
    const bar = getActiveSendBarForPanel();
    if (!bar) return;
    bar.setMultiline(!bar.isMultiline());
    updateLabel();
  });
  row.appendChild(btn);
  wrap.insertBefore(row, wrap.firstChild);
  const observer = new MutationObserver(() => {
    if (wrap.classList.contains('is-visible')) updateLabel();
  });
...
function bootApp() {
  ...
  const boot = async () => {
    ...
    if (isEmbedMode) {
      ...
      measureStartupStep('initMultilineToggleButton', () => initMultilineToggleButton());
      ...
      setSpecialCharsBarVisibility(true);
    ...
    measureStartupStep('initSettingsOpenWidgetPanelButton', () => initSettingsOpenWidgetPanelButton());
    // Terminal, Tasks, Agents, Files, Git, Widget, Instances and Status tests are
