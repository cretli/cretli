...
import { moveFolderOverlayEntry } from '../../../lib/workspace-folder-order.js';
import {
  CLONE_KEY_SEPARATOR,
...
/**
 * @param {{ variant?: string, title: string, icon: string, disabled?: boolean, onClick: () => void }} options
 * @returns {HTMLElement}
 */
...
/** File stem used when exporting a workspace to a new .code-workspace file. */
...
export function createWorkspaceSettings(deps = {}) {
  ...
  function renderFolderRows(listEl, sidebarKey, workspaceItem) {
    ...
    paths.forEach((resolvedPath, index) => {
      ...
      if (index === 0) {
        ...
        rootBadge.textContent = t('workspace.folderRootBadge');
        badges.appendChild(rootBadge);
      }
      ...
      if (paths.length > 1) {
        ...
        reorder.className = 'settings-workspace-folder-reorder';
        const moveFolder = (direction) => {
          const current = settingsWorkspaceDraftConfig[sidebarKey]?.folders || {};
          settingsWorkspaceDraftConfig[sidebarKey].folders = moveFolderOverlayEntry(
            current,
            resolvedPath,
            direction
          );
        ...
        reorder.appendChild(createIconButton({
          title: t('workspace.moveFolderUpTitle'),
          icon: 'mdi-chevron-up',
          disabled: index === 0,
          onClick: () => moveFolder('up'),
        }));
        reorder.appendChild(createIconButton({
          title: t('workspace.moveFolderDownTitle'),
  ...
  function createWorkspaceCard(workspaceItem, entry, draftConfig, cardsWrap, cardOptions = {}) {
    ...
    if (fileKind && workspaceItem.fileExists === false) {
      card.classList.add('has-missing-file');
      const missing = document.createElement('div');
      missing.className = 'settings-workspace-file-missing';
      missing.textContent = t('workspace.missingFile');
      card.appendChild(missing);
    }

    let toolsRow = null;
    if (!isClone && (fileKind || kind === 'folders')) {
      toolsRow = document.createElement('div');
      toolsRow.className = 'settings-workspace-card-tools cr-row';
      if (fileKind) {
        const convertBtn = document.createElement('cr-bar-button');
        convertBtn.textContent = t('workspace.convertToSelfLabel');
        convertBtn.title = t('workspace.convertToSelfTitle');
        convertBtn.addEventListener('click', () => {
          if (convertBtn.disabled) return;
          if (!window.confirm(t('workspace.convertToSelfConfirm', { file: workspaceFile }))) return;
          cardOptions.onStatus?.(t('common.saving'), false);
          api.convertWorkspaceFileToSelf(workspaceFile)
            .then((data) => {
              if (!data?.ok) {
                cardOptions.onStatus?.(data?.error || t('workspace.saveError'), true);
                return null;
              }
              cardOptions.onStatus?.(t('workspace.convertedToSelf'), false);
              return cardOptions.onReload?.();
            })
            .catch(() => cardOptions.onStatus?.(t('workspace.saveError'), true));
        });
        toolsRow.appendChild(convertBtn);
      } else if (kind === 'folders') {
        const exportBtn = document.createElement('cr-bar-button');
        exportBtn.textContent = t('workspace.exportToFileLabel');
        exportBtn.addEventListener('click', () => {
          if (exportBtn.disabled) return;
          const wsId = workspaceItem.id || workspaceFile;
          const stem = sanitizeFileStem(workspaceItem.name || String(wsId).split('/').pop());
          void openFsPicker({
            mode: 'folder',
            title: t('workspace.exportToFileTitle'),
            startPath: workspaceItem.workspaceDir || '~',
          }).then(async (dir) => {
            if (!dir) return;
            const targetFile = `${String(dir).replace(/\/+$/, '')}/${t('workspace.exportTargetName', { name: stem })}`;
            if (!window.confirm(t('workspace.exportConfirm', { file: targetFile }))) return;
            cardOptions.onStatus?.(t('common.saving'), false);
            try {
              const data = await api.exportWorkspaceToFile(wsId, targetFile);
              if (!data?.ok) {
                cardOptions.onStatus?.(data?.error || t('workspace.saveError'), true);
                return;
              }
              cardOptions.onStatus?.(t('workspace.exportedToFile', { file: data.file || targetFile }), false);
            } catch {
              cardOptions.onStatus?.(t('workspace.saveError'), true);
            }
          });
        });
        toolsRow.appendChild(exportBtn);
      }
      if (toolsRow.childElementCount === 0) toolsRow = null;
      else card.appendChild(toolsRow);
    }
    ...
    const foldersHint = document.createElement('p');
    foldersHint.className = 'cr-hint';
    foldersHint.textContent = t('workspace.foldersOrderHint');
    foldersField.appendChild(foldersHint);
    ...
    const addFolderBrowseBtn = document.createElement('cr-icon-button');
    addFolderBrowseBtn.className = 'settings-workspace-browse-btn';
    addFolderBrowseBtn.title = t('settings.browseFolder');
    addFolderBrowseBtn.setAttribute('aria-label', t('settings.browseFolder'));
    addFolderBrowseBtn.innerHTML = '<span class="mdi mdi-folder-search-outline" aria-hidden="true"></span>';
    addFolderBrowseBtn.disabled = entry.enabled === false;
    addFolderBrowseBtn.addEventListener('click', () => {
      if (addFolderBrowseBtn.disabled) return;
      const startPath = String(addFolderInput.value || '').trim()
        || draftConfig[sidebarKey]?.folder
        || workspaceItem.workspaceDir
        || '~';
      void openFsPicker({ mode: 'folder', startPath }).then((picked) => {
        if (!picked) return;
        addFolderInput.value = picked;
        addFolderBtn.click();
      });
    });
    addFolderRow.appendChild(addFolderBrowseBtn);
    ...
    toggle.addEventListener('change', () => {
      ...
      addFolderBrowseBtn.disabled = !toggle.checked;
      if (toolsRow) {
        toolsRow.querySelectorAll('cr-bar-button').forEach((btn) => {
  ...
  function initSettingsWorkspacePicker() {
    ...
    const addFolderBrowseBtn = document.getElementById('settings-workspace-add-folder-browse');
    ...
    const addFileBrowseBtn = document.getElementById('settings-workspace-add-file-browse');
    ...
    function renderCardsFromSettings(settingsData) {
      ...
      const cardActionOptions = {
        onReload: () => reloadAndRender(),
        onStatus: (text, isError) => setStatus(statusEl, text, isError),
      };
      ...
      list.forEach((workspaceItem) => {
        ...
        getCloneSidebarKeysForWorkspace(sidebarKey).forEach((cloneKey) => {
          ...
          createWorkspaceCard(
            ...
            settingsWorkspaceDraftConfig,
            cardsWrap,
            { sidebarKey: cloneKey, isClone: true, ...cardActionOptions }
          );
        });
      });
    ...
    addFolderBrowseBtn?.addEventListener('click', () => {
      const startPath = String(addFolderInput?.value || '').trim() || '~';
      void openFsPicker({ mode: 'folder', title: t('settings.addFolder'), startPath }).then((picked) => {
        if (picked && addFolderInput) addFolderInput.value = picked;
      });
    });
    addFileBrowseBtn?.addEventListener('click', () => {
      const startPath = String(addFileInput?.value || '').trim() || '~';
      void openFsPicker({
        mode: 'file',
        accept: '.code-workspace',
        title: t('settings.importWorkspaceFile'),
        startPath,
      }).then((picked) => {
        if (picked && addFileInput) addFileInput.value = picked;
      });
    });
