/**
 * Settings UI: CodeBuddy model catalog and chat visibility toggles (Harness tab).
 */
 ...
 */
function buildCatalogFromCodeBuddyModels(models) {
  ...
  return models
    .map((row) => {
      ...
      return {
        ...
        modelId: id,
        group: name,
        provider: id.includes('-') ? id.split('-')[0] : 'codebuddy',
      };
    })
    .filter(Boolean);
}
...
function readSettingsSortMode() {
  try {
    const stored = readStorageValueWithAlias(localStorage, CODEBUDDY_MODEL_SETTINGS_SORT_LS_KEY, '');
...
function persistSettingsSortMode(mode) {
  ...
  try {
    writeStorageValueWithAlias(localStorage, CODEBUDDY_MODEL_SETTINGS_SORT_LS_KEY, settingsSortMode);
  } catch {
    /* ignore */
  }
}
...
function syncSortSelectUi() {
  const sortEl = document.getElementById('codebuddy-model-settings-sort');
...
function renderModelSettingsList() {
  const listEl = document.getElementById('codebuddy-model-settings-list');
  const searchInput = document.getElementById('codebuddy-model-settings-search');
  if (!listEl) return;
  const query = searchInput ? String(searchInput.value || '') : '';
  const visibleRows = filterCatalogForSearch(query);
  if (visibleRows.length === 0) {
    listEl.innerHTML = '<p class="settings-hint">' + escapeHtml(t('settings.harnessCodeBuddyModelsEmpty')) + '</p>';
...
function updateToggleAllButtonLabel() {
  const btn = document.getElementById('codebuddy-model-settings-toggle-all-btn');
  if (!btn) return;
...
function updateModelSettingsSummary() {
  const summaryEl = document.getElementById('codebuddy-model-settings-summary');
  if (!summaryEl) return;
...
function readDraftEnabledKeysFromUi() {
  ...
  /** @type {Set<string>} */
  const next = new Set();
  listEl.querySelectorAll('.codebuddy-model-settings-checkbox[data-model-value]').forEach((el) => {
...
async function refreshCodeBuddyStatusPanel() {
  const statusEl = document.getElementById('codebuddy-harness-status');
  if (!statusEl) return;
  try {
    const data = await api.getCodeBuddyStatus();
    if (!data?.codebuddyApiKeyEffective) {
      statusEl.textContent = t('settings.harnessCodeBuddyNotReadyNoKey');
      return;
    }
    if (!data?.ok || !data.ready) {
      statusEl.textContent = data?.error || t('settings.harnessCodeBuddyNotReady');
      return;
    }
    statusEl.textContent = t('settings.harnessCodeBuddyReady');
  } catch {
    statusEl.textContent = t('settings.harnessCodeBuddyNotReady');
  }
}

async function loadCodeBuddyModelSettingsData() {
  ...
  void refreshCodeBuddyStatusPanel();
  try {
    const settingsData = await api.getSettings();
    const modelsData = await api.getCodeBuddyModels({ refresh: true });
    if (!modelsData?.ok) {
      ...
      if (statusEl) {
        statusEl.textContent = modelsData?.error || t('settings.harnessCodeBuddyModelsEmpty');
      }
      return;
    }
    settingsModelCatalog = enrichCatalogEntryMetaList(
      buildCatalogFromCodeBuddyModels(modelsData.models),
    );
    const enabledFromSettings = Array.isArray(settingsData?.codebuddyChatEnabledModels)
      ? settingsData.codebuddyChatEnabledModels
...
/**
 * Initializes CodeBuddy model settings panel (Settings → Harness).
 */
export function initCodeBuddyModelSettings() {
  const listEl = document.getElementById('codebuddy-model-settings-list');
  const saveBtn = document.getElementById('codebuddy-model-settings-save-btn');
  const toggleAllBtn = document.getElementById('codebuddy-model-settings-toggle-all-btn');
  const clearBtn = document.getElementById('codebuddy-model-settings-clear-btn');
  const searchInput = document.getElementById('codebuddy-model-settings-search');
  ...
  window.addEventListener('cretli-codebuddy-key-changed', () => {
    void loadCodeBuddyModelSettingsData();
  });
  listEl.addEventListener('change', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLInputElement)) return;
    if (!target.classList.contains('codebuddy-model-settings-checkbox')) return;
  ...
  saveBtn.addEventListener('click', () => {
    ...
    api.patchSettings({ codebuddyChatEnabledModels: payload }).then((data) => {
      if (!data?.ok) {
        if (statusEl) statusEl.textContent = data?.error || t('settings.chatModelsSaveError');
        return;
      }
      const savedKeys = data.codebuddyChatEnabledModels || payload;
      if (statusEl) statusEl.textContent = t('settings.chatModelsSaved');
      window.dispatchEvent(new CustomEvent('cretli-codebuddy-models-changed', {
        detail: { codebuddyChatEnabledModels: savedKeys },
      }));
    }).catch(() => {
...
export function refreshCodeBuddyModelSettingsPanel() {
  void loadCodeBuddyModelSettingsData();
}
