...
export function createSendBar(options) {
  ...
  parts.push('<span class="send-keys-end-actions">');
  if (showScreenshotButton) {
    parts.push(
  ...
  parts.push(
    '<button type="button" class="send-keys-btn" aria-label="' + escapeAttr(sendLabel) + '">' +
      '<span class="mdi mdi-send send-keys-btn-icon" aria-hidden="true"></span>' +
      '<span class="send-keys-btn-label">' + escapeHtml(sendLabel) + '</span>' +
      '</button>',
    '</span>',
    '</span>'
  );
  ...
  function getStopActionRow() {
    if (!showStop || !showExtraBarStop) return null;
    const wrap = getExtraWrapElement();
    if (!wrap) return null;
    let row = wrap.querySelector('.send-bar-stop-action-row');
    if (!row) {
  ...
  function setExtraWrapVisible(visible) {
    const wrap = getExtraWrapElement();
    if (!wrap) return;
    wrap.querySelectorAll('.send-bar-dictation-options-row').forEach((row) => {
      row.remove();
    });
    if (showExtraBarStop) {
      ...
      // This bar keeps Stop out of the ^ popover — also hide a Stop row left
      // behind by the other (terminal) send bar sharing the same extra wrap.
      wrap.querySelectorAll('.send-bar-stop-action-row').forEach((stopRow) => {
        stopRow.hidden = true;
      });
    }
