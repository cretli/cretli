...
import { createVoiceReadOptions, toggleReadAloud } from './features/voice/voiceReadControls.js';
...
export function createSendBar(options) {
  ...
  let voiceOptionsController = null;
  ...
  if (voiceBtn) {
    voiceBtn.addEventListener('click', () => {
      toggleReadAloud();
      voiceOptionsController?.refresh();
      syncVoiceButton();
    });
  ...
  function getVoiceOptionsRow() {
    if (!showVoiceReadButton) return null;
    const wrap = getExtraWrapElement();
    if (!wrap) return null;
    let row = voiceOptionsController && voiceOptionsController.root.parentNode === wrap
      ? voiceOptionsController.root
      : wrap.querySelector('.send-bar-voice-options-row');
    if (!row) {
      voiceOptionsController = createVoiceReadOptions();
      row = voiceOptionsController.root;
      row.hidden = true;
      wrap.insertBefore(row, wrap.firstChild);
    }
    voiceOptionsController?.refresh();
  ...
  function setExtraWrapVisible(visible) {
    ...
    const voiceRow = getVoiceOptionsRow();
    if (voiceRow) voiceRow.hidden = !visible;
