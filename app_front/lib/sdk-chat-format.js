...
/**
 * True when the assistant payload is a harness error dumped into the answer bubble.
 * OpenCode used to map session.error this way; the stream now uses sdkError instead.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function isHarnessErrorAssistantText(text) {
  return /^\[(OpenCode|OpenRouter|SDK) error\]\s+\S/i.test(String(text || '').trim());
}
...
export function getSdkEventTerminalChunk(chat, event) {
  ...
  if (t === 'assistant') {
    ...
    if (isHarnessErrorAssistantText(full)) return '';
