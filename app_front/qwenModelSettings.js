/**
 * Settings UI: Qwen Code model catalog and chat visibility toggles.
 */
 ...
 */
function buildCatalogFromQwenModels(models) {
...
function readSettingsSortMode() {
  try {
    const stored = readStorageValueWithAlias(localStorage, QWEN_MODEL_SETTINGS_SORT_LS_KEY, '');
...
function persistSettingsSortMode(mode) {
  ...
  try {
    writeStorageValueWithAlias(localStorage, QWEN_MODEL_SETTINGS_SORT_LS_KEY, settingsSortMode);
  } catch {
    /* ignore */
  }
}
...
function syncSortSelectUi() {
  const sortEl = document.getElementById('qwen-model-settings-sort');
...
function renderModelSettingsList() {
  const listEl = document.getElementById('qwen-model-settings-list');
  const searchInput = document.getElementById('qwen-model-settings-search');
  if (!listEl) return;
  const query = searchInput ? String(searchInput.value || '') : '';
  const visibleRows = filterCatalogForSearch(query);
  if (visibleRows.length === 0) {
    listEl.innerHTML = '<p class="settings-hint">' + escapeHtml(t('settings.harnessQwenModelsEmpty')) + '</p>';
...
function updateToggleAllButtonLabel() {
  const btn = document.getElementById('qwen-model-settings-toggle-all-btn');
  if (!btn) return;
...
function updateModelSettingsSummary() {
  const summaryEl = document.getElementById('qwen-model-settings-summary');
  if (!summaryEl) return;
...
function readDraftEnabledKeysFromUi() {
  ...
  /** @type {Set<string>} */
  const next = new Set();
  listEl.querySelectorAll('.qwen-model-settings-checkbox[data-model-value]').forEach((el) => {
...
async function refreshQwenStatusPanel() {
  const statusEl = document.getElementById('qwen-harness-status');
  if (!statusEl) return;
  try {
    const data = await api.getQwenStatus();
    if (!data?.qwenApiKeyEffective) {
      statusEl.textContent = t('settings.harnessQwenNotReadyNoKey');
      return;
    }
    if (!data?.ok || !data.ready) {
      if (data?.sdkAvailable === false) {
        statusEl.textContent = t('settings.harnessQwenNotReadySdkMissing');
        return;
      }
      statusEl.textContent = data?.error || t('settings.harnessQwenNotReady');
      return;
    }
    statusEl.textContent = t('settings.harnessQwenReady');
  } catch {
    statusEl.textContent = t('settings.harnessQwenNotReady');
  }
}

async function loadQwenModelSettingsData() {
  ...
  void refreshQwenStatusPanel();
  try {
    const settingsData = await api.getSettings();
    const modelsData = await api.getQwenModels();
    if (!modelsData?.ok) {
      ...
      if (statusEl) {
        statusEl.textContent = modelsData?.error || t('settings.harnessQwenModelsEmpty');
      }
      return;
    }
    settingsModelCatalog = enrichCatalogEntryMetaList(
      buildCatalogFromQwenModels(modelsData.models),
    );
    const enabledFromSettings = Array.isArray(settingsData?.qwenChatEnabledModels)
      ? settingsData.qwenChatEnabledModels
...
/**
 * Initializes Qwen model settings panel (Settings → Harness).
 */
export function initQwenModelSettings() {
  const listEl = document.getElementById('qwen-model-settings-list');
  const saveBtn = document.getElementById('qwen-model-settings-save-btn');
  const toggleAllBtn = document.getElementById('qwen-model-settings-toggle-all-btn');
  const clearBtn = document.getElementById('qwen-model-settings-clear-btn');
  const searchInput = document.getElementById('qwen-model-settings-search');
  ...
  window.addEventListener('cretli-qwen-key-changed', () => {
    void loadQwenModelSettingsData();
  });
  listEl.addEventListener('change', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLInputElement)) return;
    if (!target.classList.contains('qwen-model-settings-checkbox')) return;
  ...
  saveBtn.addEventListener('click', () => {
    ...
    api.patchSettings({ qwenChatEnabledModels: payload }).then((data) => {
      if (!data?.ok) {
        if (statusEl) statusEl.textContent = data?.error || t('settings.chatModelsSaveError');
        return;
      }
      const savedKeys = data.qwenChatEnabledModels || payload;
      if (statusEl) statusEl.textContent = t('settings.chatModelsSaved');
      window.dispatchEvent(new CustomEvent('cretli-qwen-models-changed', {
        detail: { qwenChatEnabledModels: savedKeys },
      }));
    }).catch(() => {
...
export function refreshQwenModelSettingsPanel() {
  void loadQwenModelSettingsData();
}
