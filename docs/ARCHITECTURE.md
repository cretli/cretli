...
| `persist/settings.js` | `data/config.json` (lanHost, workspace, additionalCursorContextDirs, cursorApiKey, ...) |
| `sdk/shared-cursor-context.js` | Extra `.cursor/` roots for SDK multi-cwd + alwaysApply prompt inject |
| `workspace.js` | Parse `.code-workspace`, list folders |
...
- The SPA shell (`/`, `/index.html`, `/embed.html`) redirects to `/login` when not
  authenticated. Static bundle assets remain public.
...
| GET | `/api/workspace` / `/api/workspaces` | Current workspace / available `.code-workspace` files |
| GET/PATCH | `/api/settings` | Runtime settings (LAN host, workspace, front HMR, cursorApiKey) |
