...
import { loadWorkspace, findWorkspaceFile } from './lib/workspace.js';
...
import { getEffectiveOpenCodeApiKey } from './lib/opencode/opencode-api-key.js';
...
import { registerDevActionsRoutes } from './lib/routes/dev-actions-routes.js';
...
const FRONT_ASSET_VERSION_FILES = [
  'index.css',
  'login.css',
  'vendor.bundle.js',
...
].map((fileName) => path.join(__dirname, 'public', 'dist', 'app', fileName));
...
function resolveFrontAssetVersion() {
  let newestMtimeMs = SERVER_STARTED_AT;
  for (const filePath of FRONT_ASSET_VERSION_FILES) {
    try {
...
function readVersionedHtmlTemplate(filePath) {
  ...
  return html.replaceAll(FRONT_ASSET_VERSION_TOKEN, resolveFrontAssetVersion());
}
