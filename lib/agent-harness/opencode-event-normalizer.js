 ...
 * @returns {string}
 */
function readSessionIdFromRecord(record) {
 ...
 * @returns {string}
 */
export function readOpenCodeEventSessionId(event) {
  if (!event || typeof event !== 'object') return '';
  const record = /** @type {Record<string, unknown>} */ (event);
  return readSessionIdFromRecord(record.properties)
    || readSessionIdFromRecord(record.data)
    || readSessionIdFromRecord(record);
}

/**
 * One OpenCode server broadcasts every workspace session. A Cretli room must
 * ignore foreign events — especially before it has its own session id, or a
 * newly opened empty chat inherits the running turn from another chat.
 *
 * @param {unknown} event
 * @param {unknown} opencodeSessionId
 * @returns {boolean}
 */
export function isOpenCodeEventForSession(event, opencodeSessionId) {
  const roomSessionId = typeof opencodeSessionId === 'string' ? opencodeSessionId.trim() : '';
  if (!roomSessionId) return false;
  const eventSessionId = readOpenCodeEventSessionId(event);
  if (!eventSessionId) return true;
  return eventSessionId === roomSessionId;
}
 ...
 * @returns {Array<Record<string, unknown>>} SDK-shaped events, or `{ kind: 'error', message }` for session.error
 */
export function normalizeOpenCodeEvent(event, context = {}) {
  if (!event || typeof event !== 'object') return [];
  if (!isOpenCodeEventForSession(event, context.opencodeSessionId)) return [];
