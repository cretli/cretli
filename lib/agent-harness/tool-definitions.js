...
import { resolveHarnessPlanPolicy } from './harness-plan-policy.js';
...
/**
 * @param {'agent' | 'plan'} mode
 * @returns {typeof OPENROUTER_AGENT_TOOLS}
 */
export function getToolsForMode(mode) {
  const policy = resolveHarnessPlanPolicy('openrouter');
  if (mode === 'plan' && policy.denyMutatingTools) {
