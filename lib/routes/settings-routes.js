...
/** @type {{ at: number, scanDir: string, payload: object|null }} */
let workspacesCache = { at: 0, scanDir: '', payload: null };
...
export function registerWorkspaceRoutes(app, ctx) {
  ...
  app.get('/api/workspaces', (_req, res) => {
    ...
    const now = Date.now();
    if (workspacesCache.payload && workspacesCache.scanDir === scanDir && now - workspacesCache.at < WORKSPACES_CACHE_TTL_MS) {
      return res.json(workspacesCache.payload);
    }
    const files = listWorkspaceFilesRecursive(scanDir);
    const toPosix = (p) => (p || '').split(path.sep).join('/');
    const list = files.map((workspaceFile) => {
      const w = loadWorkspace(workspaceFile);
      const workspaceDir = path.dirname(workspaceFile);
      return {
        workspaceFile: toPosix(path.resolve(workspaceFile)),
        name: path.basename(workspaceFile, '.code-workspace'),
        workspaceDir: toPosix(path.resolve(workspaceDir)),
        folders: w
          ? w.folders.map((f) => ({
              name: f.name,
              path: f.path,
              resolvedPath: toPosix(path.resolve(f.resolvedPath)),
            }))
          : [],
      };
    });
    const payload = { ok: true, workspaces: list };
    workspacesCache = { at: now, scanDir, payload };
