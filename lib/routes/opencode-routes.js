  ...
  app.get('/api/opencode/status', async (req, res) => {
    try {
      ...
      return res.json({
        ...
        opencodeSessionHint: 'Paste an OpenCode Zen or Z.AI API key in Settings → Harness → OpenCode, or set OPENCODE_API_KEY / ZAI_API_KEY on the server.',
        error: health.error,
      });
    } catch (err) {
    ...
    try {
      ...
      if (!health.opencodeReady) {
        return res.status(503).json({
          ok: false,
          error: health.error || 'OpenCode server is not ready — install opencode and add a Zen or Z.AI API key.',
        });
      }
