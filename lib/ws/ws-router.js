...
import { isCodeBuddyChat, isCodexChat, isDeepSeekChat, isOpenCodeChat, isOpenRouterChat, isQwenChat, isSdkChat } from '../agent-transport.js';
import {
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (isAgentSdk) {
      ...
      if (routedChat && isCodeBuddyChat(routedChat)) {
        runDetached(
          handleCodeBuddyAgentWebSocket(ws, sessionKey, {
            workspaceDirForAgent: ctx.workspaceDirForAgent,
          }),
      ...
      if (routedChat && isDeepSeekChat(routedChat)) {
        runDetached(
          handleDeepSeekAgentWebSocket(ws, sessionKey, {
            workspaceDirForAgent: ctx.workspaceDirForAgent,
          }),
      ...
      if (routedChat && isCodexChat(routedChat)) {
        runDetached(
          handleCodexAgentWebSocket(ws, sessionKey, {
            workspaceDirForAgent: ctx.workspaceDirForAgent,
          }),
          'Codex agent handler',
          ws,
      ...
      if (routedChat && isQwenChat(routedChat)) {
        runDetached(
          handleQwenAgentWebSocket(ws, sessionKey, {
            workspaceDirForAgent: ctx.workspaceDirForAgent,
          }),
          'Qwen agent handler',
          ws,
    ...
    if (isAgent && resumeId && !agentRunName) {
      ...
      if (chat && (isSdkChat(chat) || isOpenRouterChat(chat) || isOpenCodeChat(chat) || isCodeBuddyChat(chat) || isDeepSeekChat(chat) || isCodexChat(chat) || isQwenChat(chat))) {
