...
export function disposeOpenRouterRoom(sessionKey) {
  ...
  notifySdkClientsChatGone(room.clients, 'OpenRouter chat not found for this session.');
  flushPersistBuffer(room);
  openRouterRooms.delete(sessionKey);
}
...
export async function handleOpenRouterAgentWebSocket(ws, sessionKey, deps) {
  ...
  if (!chat) {
    sendSdkChatNotFoundAndClose(ws, 'OpenRouter chat not found for this session.');
    return;
  }
  if (!isOpenRouterChat(chat)) {
    if (ws.readyState === 1) {
      ws.send(JSON.stringify({
        type: 'sdkError',
   ...
   */
  async function runPrompt(text, modeOverride, displayText = '') {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'send' && typeof msg.text === 'string') {
      void runPrompt(msg.text, msg.mode, readClientDisplayText(msg));
    }
  });
