...
import { bumpChatHistoryRevision, clearChatHistoryRevision, clearChatHasPendingDelegation } from './chat-history-revisions.js';
...
export function getChatHistorySince(chatId, sinceSeq = 0, limit = HISTORY_PULL_DEFAULT_LIMIT) {
  ...
  clearChatHasPendingDelegation(chatId);
  return {
    ok: true,
    chatId: doc.chatId,
...
/**
 * HTTP query routing for GET /api/chats/:id/history.
 * `seq` is an exact event lookup (missing seq does not return a neighbor).
 * `tail` / `before` select the newest window. Otherwise `since` + `limit` reads forward.
 *
 * @param {string} chatId
 * @param {Record<string, unknown>} query
 */
export function readChatHistoryFromHttpQuery(chatId, query = {}) {
  const seqRaw = Number.parseInt(String(query?.seq || '0'), 10);
  if (Number.isInteger(seqRaw) && seqRaw > 0) {
    return getChatHistoryEvent(chatId, seqRaw);
  }
  const tailRaw = Number.parseInt(String(query?.tail || '0'), 10);
  const beforeRaw = Number.parseInt(String(query?.before || '0'), 10);
  ...
  if (wantsPage || wantsBefore) {
    return getChatHistoryPage(chatId, {
      beforeSeq: wantsBefore ? beforeRaw : 0,
      limit: wantsPage ? tailRaw : HISTORY_TAIL_DEFAULT_LIMIT,
    });
  }
  const sinceRaw = Number.parseInt(String(query?.since || '0'), 10);
  const since = Number.isFinite(sinceRaw) ? Math.max(0, sinceRaw) : 0;
  const limitRaw = Number.parseInt(String(query?.limit || String(HISTORY_PULL_DEFAULT_LIMIT)), 10);
  const limit = Number.isFinite(limitRaw)
    ? Math.min(HISTORY_PULL_MAX_LIMIT, Math.max(1, limitRaw))
    : HISTORY_PULL_DEFAULT_LIMIT;
  return getChatHistorySince(chatId, since, limit);
}

/**
 * Exact seq lookup. A gap does not return the next stored event.
 *
 * @param {string} chatId
 * @param {number} seq
 * @returns {{ ok: true, chatId: string, headSeq: number, event: { seq: number, rec: unknown } | null, events: Array<{ seq: number, rec: unknown }> }}
 */
export function getChatHistoryEvent(chatId, seq) {
  const wanted = Number(seq);
  const empty = { ok: true, chatId, headSeq: 0, event: null, events: [] };
  if (!Number.isInteger(wanted) || wanted <= 0) return empty;
  const doc = loadChatHistory(chatId);
  if (!doc) return { ...empty, chatId };
  const event = doc.events.find((row) => row.seq === wanted) || null;
  return {
    ok: true,
    chatId: doc.chatId,
    headSeq: doc.headSeq,
 ...
 * @returns {Array<{ seq: number, rec: Record<string, unknown> }>}
 */
function cloneHistoryEvents(events) {
...
/**
 * Index of the first event (in seq order) created after `upToCreatedAt` — the cut point
 * for a partial fork. Records without a parseable createdAt count as "before the fork
 * point" (they are mostly old meta rows), so they stay included up to the cut index.
 * An unparsable cutoff disables the cut (full copy).
 *
 * @param {Array<{ seq: number, rec: unknown }>} events
 * @param {string} upToCreatedAt
 * @returns {number}
 */
function resolveCutIndexByCreatedAt(events, upToCreatedAt) {
  const cutoff = Date.parse(String(upToCreatedAt || ''));
  if (!Number.isFinite(cutoff)) return events.length;
  for (let index = 0; index < events.length; index += 1) {
    const rec = events[index]?.rec;
    const createdAt =
      rec && typeof rec === 'object' ? String(/** @type {any} */ (rec).createdAt || '') : '';
    const ts = Date.parse(createdAt);
    if (Number.isFinite(ts) && ts > cutoff) return index;
  }
...
export function copyChatHistory(sourceChatId, targetChatId, targetCursorSessionId) {
  ...
  const events = source?.events ? cloneHistoryEvents(source.events) : [];
...
/**
 * Copies history into a fork up to and including the record created at `upToCreatedAt`.
 * Everything after that point stays only in the source chat. headSeq keeps the last
 * copied seq so later appends to the fork stay monotonic.
 *
 * @param {string} sourceChatId
 * @param {string} targetChatId
 * @param {string} targetCursorSessionId
 * @param {string} upToCreatedAt
 * @returns {{ ok: boolean, headSeq?: number, events?: Array<{ seq: number, rec: Record<string, unknown> }>, error?: string }}
 */
export function copyChatHistoryUntil(
  sourceChatId,
  targetChatId,
  targetCursorSessionId,
  upToCreatedAt
) {
  if (!isValidChatId(sourceChatId) || !isValidChatId(targetChatId)) {
    return { ok: false, error: 'Invalid chatId' };
  ...
  const cutIndex = resolveCutIndexByCreatedAt(all, upToCreatedAt);
  const events = cloneHistoryEvents(all.slice(0, cutIndex));
