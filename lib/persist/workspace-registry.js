...
/**
 * Replaces a `.code-workspace` registry row with a fresh folder-only (self)
 * workspace, carrying the display label over. Used by "convert to a Cretli
 * workspace" — the workspace keeps working after the file is deleted.
 *
 * @param {unknown} list
 * @param {string} fileWorkspaceId - registry id of the file workspace
 * @returns {{ registry: WorkspaceRegistryEntry[], id: string, found: boolean }}
 */
export function convertFileToFolderWorkspace(list, fileWorkspaceId) {
  const sanitized = sanitizeWorkspaces(list);
  const key = toPosixPath(fileWorkspaceId) || String(fileWorkspaceId || '').trim();
  if (!key) return { registry: sanitized, id: '', found: false };
  const index = sanitized.findIndex((entry) => entry.kind === 'file' && entry.id === key);
  if (index === -1) return { registry: sanitized, id: '', found: false };
  const label = sanitized[index].label;
  ...
  next.push(entry);
  return { registry: next, id, found: true };
}
