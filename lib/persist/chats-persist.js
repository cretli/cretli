 ...
 * @returns {{ id: string, title: string, cursorSessionId: string, workspaceFile?: string, workspaceFolder?: string, model?: string, agentTransport: 'sdk' | 'openrouter' | 'opencode', opencodeSessionId?: string, createdAt: string }}
 */
...
/**
 * Creates a persistent conversation fork with a new SDK session and the source chat metadata.
 * @param {object} parentChat
 * @param {{ workspaceFile?: string, workspaceFolder?: string }} [overrides]
 * @returns {object}
 */
export function createConversationForkChat(parentChat, overrides = null) {
  ...
  const title = `${parentChat.title || 'Chat'} (fork)`;
  ...
  return addChat(
    ...
    parentChat.model,
    {
      ...
      forkKind: 'conversation',
      summaries: parentChat.summaries,
      autoContextCompressionEnabled: parentChat.autoContextCompressionEnabled === true,
      autoContextCompressionThreshold: parentChat.autoContextCompressionThreshold,
      autoContextCompressionReset: parentChat.autoContextCompressionReset,
    }
  );
}
...
/**
 * @param {string} id - chat id (uuid)
 * @param {{ title?: string, model?: string, sdkMode?: string, sdkUiMode?: string, summary?: string, summaryTitle?: string, todoId?: string|null, archived?: boolean }} updates - fields to update; summary + summaryTitle append one entry to summaries
