...
export const REALTIME_TOOLS = [
  ...
  },
  {
    ...
    name: 'list_models',
    description:
      'List a short page of models for the active harness. Use only when the user asks what is available. Pass query to search. Prefer set_model when they already named a model.',
    parameters: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Optional spoken fragment to search (e.g. grok, gpt). Empty returns a short sample plus the current model.',
        },
      },
      additionalProperties: false,
    },
  },
  {
    ...
    name: 'set_model',
    description:
      'Change the model of the active chat. Call this immediately when the user names a model — do not call list_models first. Accepts an id or a spoken fragment. Never claim the model changed until this returns ok:true; then call get_chat_status to verify.',
    parameters: {
      type: 'object',
      properties: {
...
export function buildRealtimeInstructions(options = {}) {
  ...
  return [
    ...
    'Never say you changed the chat model, created a chat, or sent a prompt unless the matching tool returned ok:true.',
    ...
    'When the user names a chat model, call set_model immediately with that spoken name. Do not call list_models first and do not describe the catalog. Call list_models only when they ask what is available, and pass query when they name a family (grok, gpt). If set_model fails, say the error; never invent success. After ok:true, call get_chat_status and confirm the model.',
