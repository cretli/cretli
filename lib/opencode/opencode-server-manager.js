...
import { getEffectiveOpenCodeApiKey } from './opencode-api-key.js';
 ...
 * @param {number} port
 * @returns {Promise<{ client: import('@opencode-ai/sdk').OpencodeClient, server: { url: string, close(): void } } | null>}
 */
...
async function createOpenCodeInstanceEntry(workspaceFolder, port, timeout) {
  ...
  if (opencodeBin) {
    ...
    const binDir = opencodeBin.includes('/') ? opencodeBin.replace(/\/[^/]+$/, '') : '';
  ...
  const { client, server } = attached ?? await createOpencode({
  ...
  if (apiKey) {
    try {
      await applyOpenCodeZenCredentials(client, workspaceFolder, apiKey);
    } catch (err) {
 ...
 * @param {string} apiKey
 */
async function applyOpenCodeZenCredentials(client, workspaceFolder, apiKey) {
  const key = String(apiKey || '').trim();
  if (!key || !client?.auth?.set) return;
  for (const providerId of OPENCODE_ZEN_PROVIDER_IDS) {
    try {
...
export async function getOpenCodeHealth(workspaceFolder) {
  ...
  try {
    ...
    return {
      ok: true,
      opencodeReady: healthy && !!getEffectiveOpenCodeApiKey(),
      healthy,
...
/**
 * Pre-start OpenCode when a Zen key is configured (first chat/model request is faster).
 * @param {string} [workspaceFolder]
 */
export async function warmUpOpenCodeFromSettings(workspaceFolder) {
  if (!getEffectiveOpenCodeApiKey()) return { ok: false, skipped: true };
  const settings = loadSettings();
