...
export function filterCatalogByEnabled(catalog, enabledKeys) {
  ...
  const enabled = normalizeChatEnabledModels(enabledKeys);
  if (enabled.length === 0) return rows.slice();
  const allowed = new Set(enabled);
  return rows.filter((row) => {
    if (allowed.has(row.value)) return true;
    const modelId = normalizeCatalogModelValue(row.modelId);
    return Boolean(modelId && allowed.has(modelId));
  });
}
