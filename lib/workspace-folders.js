...
import { moveFolderOverlayEntry } from './workspace-folder-order.js';

export { moveFolderOverlayEntry };
...
export function mergeFoldersForClient(params) {
  ...
  for (const key of Object.keys(overlay)) pushKey(key);
  for (const key of fileByPath.keys()) pushKey(key);
  const list = [];
...
/**
 * Writes a brand-new `.code-workspace` file (or overwrites an existing one)
 * with only a `folders` array. Used by "export to file" for self-config
 * workspaces — unlike writeWorkspaceFoldersJsonc it does not require the file
 * to already exist.
 *
 * @param {string} absoluteFilePath
 * @param {Array<{ name?: string, path: string }>} folders
 */
export function writeNewWorkspaceFoldersJsonc(absoluteFilePath, folders) {
  ...
  const body = `${JSON.stringify({ folders: payload }, null, 2)}\n`;
  fs.writeFileSync(absoluteFilePath, body, 'utf8');
}
