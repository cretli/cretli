...
function tryScheduleStuckRunRecoveryRetry(room, sessionKey, text, modeOverride, clientSentAt, idleForMs, budgetMs) {
  ...
  if (shouldRetryStuckRunRecovery(stuckRetryCount)) {
    ...
    room.pendingPrompts.unshift({ text: text.trim(), mode: retryMode, clientSentAt });
    broadcastRoom(room, {
      type: 'sdkBusy',
...
function finishSdkRun(room, sessionKey, text, modeOverride, clientSentAt, input) {
  const { payload, isAuthError } = buildSdkRunFinishedPayload(room, input);
  if (
    isAuthError &&
 ...
 *
 * @param {string} sessionKey - cursorSessionId czatu SDK
 */
...
export async function handleAgentSdkWebSocket(ws, sessionKey, deps) {
   ...
   * @param {number | null} [clientSentAt]
   */
  function enqueuePrompt(text, modeOverride, clientSentAt = null) {
    const freshChat = getChatByCursorSessionId(sessionKey);
    const mode = normalizeSdkMode(modeOverride ?? freshChat?.sdkMode ?? room.sdkMode);
    const trimmed = text.trim();
    if (!trimmed) return;
    room.pendingPrompts.push({ text: trimmed, mode, clientSentAt });
    broadcastRoom(room, {
      type: 'sdkQueued',
   ...
   */
  async function runPrompt(text, modeOverride, fromQueue = false, clientSentAt = null) {
    if (!text.trim()) {
      return;
    }
    ...
    if (room.busy) {
      enqueuePrompt(text, modeOverride, clientSentAt);
      return;
    }
    if (!fromQueue) {
      resetSdkAutomaticRetryCounters(room);
    ...
    try {
      ...
      if (room.currentRun) {
        ...
        if (broadcastSdkRateLimitError(room, message)) {
          clearCurrentRunDiagnostics(room);
          return;
        }
      ...
      if (next) {
        schedulePendingPromptRun(next, (item) => {
          void runPrompt(item.text, item.mode, true, item.clientSentAt ?? null);
        });
      } else {
        setImmediate(() => {
          if (room.busy) return;
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'send' && typeof msg.text === 'string') {
      void runPrompt(msg.text, msg.mode, false, msg.clientSentAt);
      return;
    }
