...
import { isExternalMcpToolName } from '../mcp/mcp-policy.js';
 ...
 * @returns {string}
 */
export function resolveReadOnlyGuardUserMessage(mode) {
...
export function resolvePlanModeSdkEventDecision(input = {}) {
  ...
  if (!isReadOnlySdkMode(input.mode)) return idle;
  const policy = resolveHarnessReadOnlyPolicy(input.transport, input.mode);
  if (!policy.denyMutatingTools) return idle;
  const eventName = input.event && typeof input.event === 'object'
    ? /** @type {{ name?: unknown }} */ (input.event).name
    : '';
  if (isExternalMcpToolName(eventName)) return idle;
